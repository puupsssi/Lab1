...
int main() {
    ...
    int p = 0;
    // Начальные условия
    double x0, right_boarder;  // [x(0), right_boarder]
    ...
    double epsilon_boarder = 0.01;//в какой окрестности от границы мы должны завершить счет
    ...
    case 0:
        ...
        result = runge_kutta_4th_order(f0, x0, v0, h, n_steps, epsilon, need_epsilon,right_boarder,epsilon_boarder, &changes_of_the_step, &for_test_task);
    ...
    case 1:
        ...
        f1 = f_first_task;
        //Вызываем метод Рунге-Кутта
        result = runge_kutta_4th_order(f1, x0, v0, h, n_steps, epsilon, need_epsilon, right_boarder, epsilon_boarder, &changes_of_the_step, &for_test_task);
    ...
    case 2:
        ...
        f2 = f_second_task;
        //Вызываем метод Рунге-Кутта
        result = runge_kutta_4th_order_for_system(f2, x0, v_10,v_20,A,B, h, n_steps, epsilon, need_epsilon, right_boarder, epsilon_boarder, &changes_of_the_step);
